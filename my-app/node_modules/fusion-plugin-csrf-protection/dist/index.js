...
const CsrfExpireToken = fusionCore.createToken('CsrfExpireToken');
const CsrfIgnoreRoutesToken = fusionCore.createToken('CsrfIgnoreRoutesToken');
const FetchForCsrfToken = fusionCore.createToken('FetchForCsrfToken');
...
const plugin = true && fusionCore.createPlugin({
  deps: {
    Session: fusionTokens.SessionToken,
    ...
    return async function csrfMiddleware(ctx, next) {
      if (ctx.path === '/csrf-token' && ctx.method === 'POST') {
        ...
        if (ctx.element) {
          ...
          ctx.template.body.push(fusionCore.html`<script id="__CSRF_TOKEN__" type="application/json">${JSON.stringify(token)}</script>`);
        }
